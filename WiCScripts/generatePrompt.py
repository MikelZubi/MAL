...
def generate_promptV2(modelname, tokenizer,word, example, few = {"k":0}):
    ...
    if data["type"] == "Instruct":
        chat = []
        for i in range(few["k"]):
            chat.append({"role": "user", "content": "As an expert English lexicographer generate a dictionary definition of the word '" + few["words"][i] + "' in the sense of this example '" + few["examples"][i][0] + "'. Give JUST the definition not other things."})
        ...
        for i in range(few["k"]):
            chat.append({"role": "user", "content": "Giving the word '" + few["words"][i] + "' and the sense of this example: '" + few["examples"][i][0] + "', generate the definition of the word in this sense. Give JUST the definition not further explanation."})
            chat.append({"role": "assistant", "content": few["definitions"][i]})
...
def generate_promptExamples(modelname, tokenizer, word, example, kE = 8, few = {"k":0}):
    f = open('modelsData.json')
    k = str(kE)
    data = json.load(f)[modelname]
    if data["type"] == "Instruct":
        chat = []
        for i in range(few["k"]):
            chat.append({"role": "user", "content": "As an expert English lexicographer generate and enumerate "+k+" examples of usage of the word '" + few["words"][i] + "' in the sense of this example '" + few["examples"][i][0] + "'."})
            chat.append({"role": "assistant", "content": enumerate_examples(few["examples"][i][1:])})
        
        chat.append({"role": "user", "content": "As an expert English lexicographer generate and enumerate "+k+" examples of the word '" + word + "' in the sense of this example '" + example +"'."})
    elif data["type"] == "Chat":
        ...
        for i in range(few["k"]):
            chat.append({"role": "user", "content": "Giving the word '" + few["words"][i] + "' and the sense of this example: '" + few["examples"][i][0] + "', generate another "+ k +" usage examples."})
            chat.append({"role": "assistant", "content": enumerate_examples(few["examples"][i][1:])})
        chat.append({"role": "user", "content": "Giving the word '" + word + "' and the sense of this example: '" + example + "', generate another "+ k +" usage examples."})
    else:
        assert False
    prompt = tokenizer.apply_chat_template(chat, tokenize=False, add_generation_prompt=True)
    return prompt
...
def generate_promptExampleDef(modelname, tokenizer, word, example, definition, kE = 8, few = {"k":0}):
    f = open('modelsData.json')
    k = str(kE)
    data = json.load(f)[modelname]
    if data["type"] == "Instruct":
        chat = []
        for i in range(few["k"]):
            chat.append({"role": "user", "content": "As an expert English lexicographer generate and enumerate "+k+" examples of usage of the word '" + few["words"][i] + "' in the sense of this definition: '" + few["definitions"][i] + "' and also giving this example: '" + few["examples"][i][0] + "'."})
            chat.append({"role": "assistant", "content": enumerate_examples(few["examples"][i][1:])})
        
        chat.append({"role": "user", "content": "As an expert English lexicographer generate and enumerate "+k+" examples of the word '" + word + "' in the sense of this definition: '" + definition + "' and using this example sentence: '" + example + "'."})
    elif data["type"] == "Chat":
        ...
        for i in range(few["k"]):
            chat.append({"role": "user", "content": "Giving the word '" + few["words"][i] + "' and the sense of this definition: '" + few["definitions"][i] + "' and using this example sentence: '" + few["examples"][i][0] + "', generate another "+ k +" usage examples."})
            chat.append({"role": "assistant", "content": enumerate_examples(few["examples"][i][1:])})
        chat.append({"role": "user", "content": "Giving the word '" + word + "' and the sense of this definition: '" + definition + "' and using this example sentence: '" + example + "', generate another "+ k +" usage examples."})
    else:
        assert False
    prompt = tokenizer.apply_chat_template(chat, tokenize=False, add_generation_prompt=True)
    return prompt